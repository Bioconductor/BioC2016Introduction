...
output:
  BiocStyle::html_document:
    toc: true
vignette: >
  % \VignetteIndexEntry{Lab 1: Introduction to R}
...
Authors: Valerie Obenchain (<a
  href="mailto:valerie.obenchain@roswellpark.org">valerie.obenchain@roswellpark.org.org</a>),
  Lori Shepherd (<a
  href="mailto:lori.shepherd@roswellpark.org">lori.shepherd@roswellpark.org</a>),
  Martin Morgan (<a
...
fl <- system.file(package="BioC2016Introduction", "extdata", "symgo.csv")
```
...
```
```{r echo=FALSE}
fname <- system.file(package="BioC2016Introduction", "extdata",
    "ALLphenoData.tsv")
stopifnot(file.exists(fname))
pdata <- read.delim(fname)
```
